...
from job_env import delete_trade_date_range, fetch_open_trade_dates, resolve_date_window
...
def main():
    ...
    try:
        ...
        if full_refresh:
            delete_trade_date_range(
                cursor, "stock_daily_history", min(trade_dates), max(trade_dates)
            )
            conn.commit()
