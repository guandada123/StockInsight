...
describe("useMarketStore", () => {
  ...
  it("多次调用 setIndices 会覆盖之前的数据", () => {
    useMarketStore.getState().setIndices({ "000001": { name: "旧数据", code: "000001", price: 3000, change: 0, change_pct: 0 } });
    useMarketStore.getState().setIndices({ "399006": { name: "创业板指", code: "399006", price: 2500, change: 10, change_pct: 0.4 } });
  ...
  it("setIndices 接收空对象时，indices 被清空", () => {
    useMarketStore.getState().setIndices({ "000001": { name: "上证", code: "000001", price: 3200, change: 0, change_pct: 0 } });
...
describe("useAnalysisStore", () => {
  ...
  it("setResult 设置分析结果", () => {
    const mockResult: StockAnalysisResult = {
      ...
      quote: { code: "600519", name: "贵州茅台", price: 1500, open: 1498, high: 1510, low: 1490, prev_close: 1500, change: 0, change_pct: 0, amplitude: 1.5 },
      technical: { ma_status: "多头排列", macd_signal: "金叉", kdj_signal: "向上", rsi_value: 55, atr: 20, support: [1480], resistance: [1520], stop_loss: 1480, stop_profit: 1520 },
      quant: { composite: 75, rating: "A", factor_scores: { momentum: 80, technical: 70, fundamental: 85, volume: 65, risk: 75 } },
      ...
      near_5d: 3.5, near_20d: 8.2,
      short_score: 70, long_score: 80, style: "价值成长",
    };
  ...
  it("setResult(null) 可以清除结果", () => {
    const mockResult: StockAnalysisResult = {
      code: "600519", name: "测试", time: "",
      quote: { code: "", name: "", price: 0, open: 0, high: 0, low: 0, prev_close: 0, change: 0, change_pct: 0, amplitude: 0 },
      technical: { ma_status: "", macd_signal: "", kdj_signal: "", rsi_value: 0, atr: 0, support: [], resistance: [], stop_loss: 0, stop_profit: 0 },
      quant: { composite: 0, rating: "", factor_scores: { momentum: 0, technical: 0, fundamental: 0, volume: 0, risk: 0 } },
      ...
      near_5d: 0, near_20d: 0, short_score: 0, long_score: 0, style: "",
    };
    useAnalysisStore.getState().setResult(mockResult);
...
describe("usePortfolioStore", () => {
  ...
  it("setData 更新持仓数据", () => {
    const mockPortfolio: PortfolioData = {
      name: "我的组合",
      holdings: [
        { code: "600519", name: "贵州茅台", shares: 100, cost: 1450, current_price: 1500, market_value: 150000, profit_amount: 5000, profit_pct: 3.45, weight_pct: 50 },
        { code: "300750", name: "宁德时代", shares: 200, cost: 180, current_price: 190, market_value: 38000, profit_amount: 2000, profit_pct: 5.56, weight_pct: 50 },
      ],
      total_value: 188000,
      total_cost: 181000,
      total_profit: 7000,
