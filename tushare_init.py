    ...
    for td in trade_dates:
        ...
        for _, r in df.iterrows():
            ...
            bt = sum(
                ...
                for c in ["buy_sm_amount", "buy_md_amount", "buy_lg_amount", "buy_elg_amount"]
            )
            st = sum(
                float(r.get(c, 0) or 0)
                for c in ["sell_sm_amount", "sell_md_amount", "sell_lg_amount", "sell_elg_amount"]
            )
            rows.append(
                (
                    code,
        ...
        conn.executemany(
            "INSERT OR REPLACE INTO stock_moneyflow VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)", rows
        )
        conn.commit()
        total += len(rows)
