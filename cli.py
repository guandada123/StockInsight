...
示例:
  ...
  python cli.py check --owned
    ...
    esp.add_argument(
        "--mode",
        choices=["mainboard", "full", "top_sectors"],
        default="mainboard",
    ...
    ap.add_argument(
        "--full", action="store_true", help="一键全出：技术+量化+基本面+资金+回测+AI+宏观"
    )
    ap.add_argument(
        "--fast", action="store_true", help="快速模式：仅本地计算(L0-L3)，零网络等待，盘中秒出"
    )
    ap.add_argument(
        "--ultimate",
        action="store_true",
        help="终极分析：大盘→板块排名→资金流向→七层→预测价位→操作建议",
    )
    ...
    rp.add_argument(
        "--type",
        choices=["screener", "eod", "sector", "portfolio"],
        default="eod",
        help="报告类型 (默认eod)",
    )
    ...
    cp.add_argument(
        "--premarket", action="store_true", help="开盘前全面自检: 网络+数据库+数据源+API一键检测"
    )
    ...
    cfgp.add_argument(
        "action", choices=["show", "init"], default="show", help="show=查看配置, init=生成默认配置"
    )
    ...
    avp.add_argument(
        "--insider", type=str, nargs="?", const="all", help="高管增减持(指定代码或全部)"
    )
...
def _cmd_premarket_check():
    ...
    try:
        ...
        for src_name, label in [("sina", "新浪"), ("tencent", "腾讯"), ("eastmoney", "东方财富")]:
            ...
            if s:
                ...
                lat = s.latency_ms
                if avail:
                    check(label, True, f"{lat:.0f}ms")
                else:
                    check(label, False, "不可达" if lat > 5000 else f"{lat:.0f}ms")
        # Baostock
        try:
            ...
            check(
                "Baostock",
                lg.error_code == "0",
                "本地库可用" if lg.error_code == "0" else lg.error_msg,
            )
            bs.logout()
        except Exception:
    ...
    try:
        ...
        if ff is not None and not ff.empty:
            ...
            if now_h < 15:
                print("  ⚠️ 东方财富资金流向: 盘中无数据，盘后T+1发布 (正常)")
            else:
    ...
    if results["fail"] == 0:
        print("  结论: 🟢 系统就绪，可以正常使用")
    elif results["fail"] <= 2:
        print("  结论: 🟡 部分数据源不可用，基本功能正常")
    else:
        print("  结论: 🔴 多个数据源故障，建议等待恢复后重试")
    print(f"{'=' * W}\n")
...
def cmd_scan(args):
    ...
    print(f"{'=' * 60}")
    print(
        f"扫描模式: {'全A股' if args.mode == 'full' else '仅主板'} | "
    ...
    print(f"\n{'=' * 60}")
    print(
        f"扫描完成: {len(results)} 只达标  跳过/失败: {skipped}  耗时 {((time.time() - t0) / 60):.0f}min"
    )
    print(f"{'=' * 60}")
    print(
        f"{'#':<4} {'代码':<8} {'板块':<10} {'综合':<7} {'近20日':<9} {'RSI':<5} {'MACD':<6} {'基本面':<6}"
    )
    print("-" * 72)
...
def _print_full_analysis(code, days=365, fast=False):
    ...
    try:
        ...
        print(
            f"  换手率: {turnover.get('换手率%', 0):.1f}% | 量比: {turnover.get('量比', 1):.1f} | {turnover.get('信号', '')}"
        )
        print(
            f"  连{consec.get('方向', '')}{consec.get('天数', 0)}天 | {consec.get('信号', '')} | 节奏: {tail.get('近5日涨跌节奏', '')}"
        )
        print(
            f"  尾盘: {tail.get('尾盘倾向', '')} | 近10日阳线: {tail.get('近10日阳线占比%', 50):.0f}%"
        )
        print(
            f"  短线评分: {st_score.get('短线评分', 0):.0f} → {st_score.get('评级', '')} | ATR占比: {st_score.get('ATR占比%', 0):.1f}%"
        )
        # 组合信号
        print(
            f"  组合信号: {combo.get('信号', '')} (强度{combo.get('强度', 0):+d}) | {' '.join(combo.get('详情', [])[:4])}"
        )
        if not fast:
        ...
        ff = (
            calc_fund_flow_summary(code)
            if not fast
            else {"主力动向": "快速模式跳过", "近5日累计(万)": 0, "今日方向": ""}
        )
        print(
            f"  主力: {ff.get('主力动向', '')} | 近5日: {ff.get('近5日累计(万)', 0)}万 | 今日: {ff.get('今日方向', '')}"
        )
    ...
    n20 = r["near_20d"]
    n60 = (
        float((kline.iloc[-1]["收盘"] / kline.iloc[-61]["收盘"] - 1) * 100)
        if len(kline) > 60
        else 0
    )
    sep("L1 技术面")
    print(
        f"  现价: {price:.2f}  |  近5日: {n5:+.1f}%  |  近20日: {n20:+.1f}%  |  近60日: {n60:+.1f}%"
    )
    ...
    sep("L2 量化评分")
    print(
        f"  综合: {r['qs_composite']:.0f} → {r['qs_rating']}  |  短线: {r['short_score']:.0f}分  |  长线: {r['long_score']:.0f}分  |  风格: {r['style']}"
    )
    print(f"  动量: {r['mom_s']:.0f}  技术: {r['tech_s']:.0f}  基本面: {r['fund_s']:.0f}")
    print(f"  量能: {r['vol_s']:.0f}  风险: {r['risk_s']:.0f}")
    print(
        f"  夏普: {r['sharpe']:.2f}  |  回撤: {r['max_dd']:.1f}%  |  波动率: {r['volatility']:.1f}%"
    )
    ...
    if r["has_nt"]:
        ...
        print(
            f"  国家队: 🏛️ {', '.join(holders[:5])}{'...' if len(holders) > 5 else ''} ({len(holders)}家)"
        )
    ...
    try:
        ...
        print(
            f"  📊 结论: {debate['verdict']} → {debate['action']} (置信度:{debate['confidence']})"
        )
    except Exception as e:
        print(f"  (多空辩论: {e})")
    ...
    sep("L4 资金面 & 消息")
    try:
        from stock_analyzer.advanced import (
            get_institution_visits,
            get_margin_summary,
            insider_signal,
            north_flow_signal,
        )
        ...
        if "error" not in nf:
            print(
                f"  北向资金: {'净流入' if any(v.get('净买额(亿)', 0) > 0 for v in nf.values()) else '净流出'}"
            )
    ...
    sep("L5 策略回测")
    try:
        from stock_analyzer.backtest import DEFAULT_COMPARE_STRATEGIES, compare_strategies
        ...
        if bt:
            ...
            print(
                f"  最优策略: {bt[best[0]]['name']} (夏普{best[1]['metrics']['夏普比率']:.2f} 超额{best[1]['metrics']['超额收益%']:+.1f}%)"
            )
    ...
    try:
        ...
        if agreement == "数据不足":
            ...
            print(
                f"  {emoji} 三模型一致{direction}  |  置信度{confidence:.0f}%  |  一致性:{agreement} ({votes})"
            )
        elif agreement == "中":
            print(
                f"  ⚠️ 模型有分歧  |  投票: {direction}  |  置信度{confidence:.0f}%  |  一致性:中 ({votes})"
            )
        else:
            print(
                f"  模型预测方向: {direction}  |  置信度{confidence:.0f}%  |  一致性:低 ({votes})"
            )
...
def cmd_enhanced_scan(args):
    ...
    print(f"{'=' * W}")
    print(
        f"增强选股: mode={args.mode} top_n={args.top_n} "
    ...
    if args.type == "eod":
        ...
        for code, info in owned.items():
            ...
            if r:
                ...
                print(
                    f"  {status} {code} {r['name']}: 浮盈{pnl:+.2f}% | "
...
def cmd_check(args):
    ...
    if args.market or (not args.owned and not args.alerts):
        ...
        for code, info in mkt.items():
            print(f"  {info['名称']}: {info['最新价']:.2f} ({info['涨跌幅']:+.2f}%)")
    ...
    if args.rs:
        ...
        try:
            ...
            for code in codes:
                ...
                print(
                    f"  {code}: 个股{rs['stock_return']:+.1f}%  大盘{rs['index_return']:+.1f}%  "
...
def cmd_history(args):
    ...
    if args.summary:
        ...
        if s:
            ...
            print(
                f"优秀(≥80): {s['excellent']} 只 ({s['excellent'] / s['total'] * 100:.1f}%)"
                if s["total"]
                else ""
            )
            print(
                f"良好(60-79): {s['good']} 只 ({s['good'] / s['total'] * 100:.1f}%)"
                if s["total"]
                else ""
            )
            print(
                f"中等(40-59): {s['medium']} 只 ({s['medium'] / s['total'] * 100:.1f}%)"
                if s["total"]
                else ""
            )
            print(
                f"偏低(<40): {s['low']} 只 ({s['low'] / s['total'] * 100:.1f}%)"
                if s["total"]
                else ""
            )
        else:
    ...
    if args.code:
        ...
        if df.empty:
            ...
            print(
                f"{'日期':<12} {'综合':<7} {'动量':<6} {'技术':<6} {'基本面':<6} {'量能':<6} {'风险':<6} {'价格':<8}"
            )
            print("-" * 65)
            for _, r in df.iterrows():
                print(
                    f"{r['date']:<12} {r['composite_score']:<7.1f} {r['momentum_score']:<6.0f} "
    ...
    if df.empty:
        ...
        print(f"\n{date} Top {len(df)}:")
        print(
            f"{'#':<4} {'代码':<8} {'名称':<8} {'综合':<7} {'动量':<6} {'技术':<6} {'基本面':<6} {'价格':<8}"
        )
        print("-" * 65)
        for i, (_, r) in enumerate(df.iterrows()):
            print(
                f"{i + 1:<4} {r['code']:<8} {r['name']:<8} {r['composite_score']:<7.1f} "
...
def cmd_clean(args):
    ...
    patterns = [
        "deep_analysis_*.json",
        "final_5_analysis*.json",
        "short_term_*_analysis.json",
        "rational_*.json",
        "sector_copper_result.json",
        "candidate_pool_*.csv",
        "screener_*.csv",
        "deep_dive_*.json",
    ]
...
def _print_stock_analysis(r):
    ...
    print(
        f"  🎯 因子: 动量{r['mom_s']:.0f} 技术{r['tech_s']:.0f} 基本面{r['fund_s']:.0f} "
    ...
    if args.lhb:
        ...
        if "error" in result:
            ...
            if top:
                ...
                for r in top:
                    print(
                        f"  {r.get('代码', ''):<8} {r.get('名称', ''):<8} {r.get('净买额', 0):<12.0f} {r.get('涨幅', 0):<8}%"
                    )
    ...
    if args.margin:
        ...
        if result:
            for mkt, data in result.items():
                print(
                    f"  {mkt}: 融资{data.get('融资余额(亿)', 0):.0f}亿 融券{data.get('融券余额(亿)', 0):.0f}亿"
                )
        else:
            print("  数据获取失败")
...
def cmd_backtest(args):
    ...
    print(
        f"数据: {len(kline)} 天  |  初始资金: {args.capital:,}  |  "
    ...
    if args.compare:
        ...
        for s_name, r in results.items():
            m = r["metrics"]
            print(
                f"{r['name']:<16} {m['总收益率%']:<10.1f} {m['超额收益%']:<8.1f} "
        ...
        if result:
            ...
            print(
                f"夏普: {m['夏普比率']:.2f}  索提诺: {m['索提诺比率']:.2f}  卡玛: {m['卡玛比率']:.2f}"
            )
            print(
                f"回撤: {m['最大回撤%']:.1f}%  胜率: {m['胜率%']:.0f}%  交易: {m['交易次数']}次  盈亏比: {m['盈亏比']:.2f}"
            )
            ...
            if result["trades"]:
                ...
                for t in result["trades"][-5:]:
                    ...
                    reason = (
                        f" [{t.get('exit_reason', '')}]"
                        if t.get("exit_reason") and t["action"] == "SELL"
                        else ""
                    )
...
def cmd_ml(args):
    """机器学习预测"""
    from stock_analyzer.cache import cached_fundamentals, cached_kline
    from stock_analyzer.ml_predict import ml_enhanced_score, predict_direction, predict_return
...
def cmd_quality(args):
    ...
    if cf["operating_cf_yi"]:
        print(
            f"  经营CF: {cf['operating_cf_yi']}亿 | 投资CF: {cf['investing_cf_yi']}亿 | 融资CF: {cf['financing_cf_yi']}亿"
        )
