...
def generate(code: str):
    ...
    from stock_analyzer.fetcher import get_market_overview, sina_real_time
    ...
    from stock_analyzer.quant import calc_risk_metrics, composite_quant_score
    ...
    debate = generate_bull_bear_debate(
        {
            ...
            "technical": {
                ...
                "resistance": sr.get("压力位", []),
                "price": price,
                "pe": 0,
            },
            "fund_flow": {"direction": "无数据"},
            "ai_prediction": {
                "direction": ml.get("ensemble_direction", "?"),
                "confidence": ml.get("ensemble_confidence", 50),
            },
        }
    )
    try:
        ...
        total_flow = 0
    try:
        bt = compare_strategies(kline, DEFAULT_COMPARE_STRATEGIES, 100000)
    except:
        bt = {}
    try:
    ...
    rr = rl.add_run(f"综合风险评级: {risk_level}")
    rr.font.size = Pt(18)
    rr.bold = True
    rr.font.color.rgb = RGBColor(
        int(risk_color[0:2], 16), int(risk_color[2:4], 16), int(risk_color[4:6], 16)
    )
    ...
    doc.add_paragraph(
        f"报告日期: {datetime.now().strftime('%Y-%m-%d')} (K线数据至 {str(kline.iloc[-1][chr(26085) + chr(26399)])[:10]}, 实时价 {price:.2f})"
    )
    doc.add_page_break()
    ...
    doc.add_paragraph(
        f"{name}（{code}）当前股价 {price} 元，近5日涨跌幅 {n5:+.1f}%。"
    ...
    doc.add_paragraph(
        "技术面通过价格和成交量的历史数据，判断股票当前处于什么状态。", style="Intense Quote"
    )
    add_kpi_table(
        ["指标", "当前值", "什么意思", "信号"],
        [
            ["最新价", f"{price:.2f} 元", "股票的当前市场价格", "—"],
            [
                "今日区间",
                f"{float(info.get('今开', 0)):.2f} - {float(info.get('最高', 0)):.2f}",
                "今天最高到最低的范围",
                "—",
            ],
            [
                "近5日涨跌",
                f"{n5:+.1f}%",
                "最近一周的涨跌。涨太多追高风险大，跌太多可能反弹",
                "⚠️ 追高" if n5 > 12 else ("✅ 温和" if n5 > 0 else "🟢 超跌反弹机会"),
            ],
            [
                "近20日涨跌",
                f"{n20:+.1f}%",
                "最近一个月的涨跌。超过30%说明涨太多了",
                "🔴 过热" if n20 > 30 else ("✅ 正常" if n20 > -10 else "🟢 超跌"),
            ],
            [
                "RSI",
                f"{rsi_val:.0f}",
                "强弱指标。>70太贵(超买)，<30太便宜(超卖)",
                f"{'🔴 超买' if rsi_val > 70 else ('🟢 超卖机会' if rsi_val < 30 else '✅ 正常')}",
            ],
            [
                "MACD",
                macd_sig,
                "趋势方向。多头=上涨趋势，空头=下跌趋势",
                "✅ 向好" if "多头" in macd_sig else "⚠️ 偏弱",
            ],
        ],
    )
    ...
    doc.add_paragraph(
        "我们用7个因子给股票打分（满分100），综合考虑了涨跌幅、技术指标、基本面、成交量、风险等。"
    )
    ...
    add_kpi_table(
        ...
        [
            [
                "📈 动量",
                f"{qs_fs('momentum', quant):.0f}",
                "100",
                "近期涨幅大小。分太高=涨太多了(追高风险)，分太低=跌太多了(可能反弹)",
            ],
            [
                "🔧 技术",
                f"{qs_fs('technical', quant):.0f}",
                "100",
                "MACD+RSI+KDJ等技术指标的综合判断",
            ],
            [
                "💰 基本面",
                f"{qs_fs('fundamental', quant):.0f}",
                "100",
                "ROE/毛利率/营收增长等财务指标",
            ],
            [
                "📊 量能",
                f"{qs_fs('volume', quant):.0f}",
                "100",
                "成交量和价格配合度。放量上涨好，缩量下跌差",
            ],
            ["⚠️ 风险", f"{qs_fs('risk', quant):.0f}", "100", "波动率和回撤。分越低=风险越大"],
            [
                "🎯 综合",
                f"{qs:.0f}",
                "100",
                f"评级: {quant.get('rating', '')}。≥80强烈买入，≥60可以买，<40建议观望",
            ],
        ],
    )
    ...
    doc.add_paragraph()
    doc.add_paragraph(
        f"短线评分: {st_sc:.0f} 分（{st.get('评级', '') if isinstance(st, dict) else ''}）  "
    ...
    add_kpi_table(
        ...
        [
            [
                "ROE(净资产收益率)",
                f"{roe:.2f}%",
                "巴菲特最看重的指标。>15%优秀，>10%良好，<5%偏弱。ROE高的公司更能赚钱",
            ],
            ["PE(市盈率)", f"{pe}", "股价÷每股收益。PE越低越便宜。成长股PE较高，价值股PE较低"],
            ["PB(市净率)", f"{pb}", "股价÷每股净资产。PB<1可能被低估，银行股通常PB较低"],
            ["每股收益", f"{eps}", "每持有一股能分到多少利润。数值越高越好"],
            [
                "主力资金(5日)",
                f"{total_flow:+.2f}亿",
                "大资金最近5天是买入还是卖出。正数=大资金在买，负数=在卖",
            ],
        ],
    )
    ...
    doc.add_paragraph(
        "AI用三个模型（XGBoost/RandomForest/LightGBM）分析历史数据，预测股票短期涨跌。"
    )
    ...
    if debate["bull"]["score"] > debate["bear"]["score"]:
        ...
        doc.add_paragraph("👉 结论: 双方势均力敌 — 建议观望等待方向明确")
    ...
    action_text = (
        "可以买入" if buy_signals >= 4 else ("轻仓试探" if buy_signals >= 2 else "建议观望或减仓")
    )

    p = doc.add_paragraph()
    pr = p.add_run(f"📌 {action_text}")
    pr.font.size = Pt(16)
    pr.bold = True
    pr.font.color.rgb = RGBColor(
        int(action_color[0:2], 16), int(action_color[2:4], 16), int(action_color[4:6], 16)
    )
    ...
    add_kpi_table(
        ...
        [
            [
                "🟢 买入区间",
                f"{buy_low} - {buy_high} 元",
                f"在这个价格范围内买入比较安全。当前价{price}元{'在区间内，可以考虑' if buy_low <= price <= buy_high else '，等回调到区间再买'}",
            ],
            [
                "🔴 止损价",
                f"{stop.get('止损参考价', price * 0.93):.2f} 元",
                "如果跌破这个价格，说明判断错了，必须卖出止损。亏小钱比亏大钱好",
            ],
            [
                "🎯 止盈价",
                f"{stop.get('止盈参考价', price * 1.07):.2f} 元",
                "涨到这个价格附近可以考虑卖出获利。不要贪心，有赚就好",
            ],
            ["⏱️ 建议持有", "3-5 个交易日", "短线操作不要太久，快进快出"],
        ],
    )
    ...
    doc.add_paragraph(
        "💡 小贴士："
        "止损价是最后的底线，到了必须走；止盈价是参考目标，到了可以考虑卖。"
        "如果买入后3天还没涨，也应该考虑退出。"
    )
    ...
    if macro and "error" not in macro:
        ...
        ind = macro.get("数据", {})
        doc.add_paragraph(
            f"PMI: {ind.get('制造业PMI', '?')}（>50经济扩张，<50经济收缩）  "
    ...
    p.add_run(
        "本报告由AI自动生成，所有分析仅供参考学习，不构成任何投资建议。"
        "股市有风险，投资需谨慎。请根据自身情况独立做出投资决策。"
    )
    doc.add_paragraph(
        "数据来源: 新浪财经(实时行情) | Baostock(行业分类) | akshare(基本面) | 东方财富(板块资金)"
    )
    doc.add_paragraph(
        f"数据时间: K线来源 {str(kline.iloc[-1][chr(26085) + chr(26399)])[:10]}  |  实时行情 {datetime.now().strftime('%H:%M')} (非交易日为上一交易日)"
    )
